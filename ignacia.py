...
def actualizar_pokemon(nombre):
    for pokemon in pokedex:   # gracias al for buscamos el pokemon 
        if pokemon["nombre"].lower() == nombre.lower():  #aki comparamos sin mayuscula para que no se caiga el sistema al escribir mal
            nuevo_tipo = input("nuevo tipo: ").strip() #aka le pide al usuario un nuevo tipo
...
while True:
    ...
    if opcion == "1":
        ...
        if nombre == "":
            print("nombre vacío")
        elif not nombre.replace(" ", "").isalpha(): # Valida que el nombre solo tenga letras (permite espacios entre palabras)
        ...
        nombre = input("nombre del pokemon: ").strip() # pide nombre 
        if nombre == "":
            print("nombre vacío.")
        else:
